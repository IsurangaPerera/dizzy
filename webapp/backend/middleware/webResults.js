...
const webResults = asyncHandler(async (request, response, next) => {
      ...
      query: {
        bool: {
          must: {
            multi_match: {
              query: query,
              fields: ["source", "info.tags.cryptocurrency.address.btc", "summary", "info.tags.abuse.report.type",
  ...
  const hits = results.body.hits.hits
      .map((hit) => {
        ...
        return {
          ...
          body: (hit._source.info.tags.abuse) ? hit._source.info.tags.abuse.report.description : hit._source.summary
              || hit._source.info.title,
          info: [
            {
              title: "Safety",
              text: "N/A",
  ...
  if (page === 1 && (query.toUpperCase() === "wannacry".toUpperCase() ||
    ...
    hits.unshift({
      ...
      body: "Daily Stormer The Most Censored Publication in HistoryAmerican Government Blames the WannaCry Attack on " +
    ...
    hits[2] = {
      ...
      crawledAt: "2020-12-02T13:57:41.784+00:00",
      body: "From the moment you read this letter, after 60 hours, all your contacts on this email box and in your " +
